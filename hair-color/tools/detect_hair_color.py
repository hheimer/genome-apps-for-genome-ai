...
def get_genome_app_data():
    ...
    header = [
        ...
        "# biology.description=The biological process responsible for hair color."
    ]
    ...
    data[
        'biology'] = "Hair color is the result of two types of melanin, eumelanin and pheomelanin."
...
def detect_hair_color():
    ...
    p_brown = exp(dot(data.iloc[:, 3], input_vector))
    p_red = exp(dot(data.iloc[:, 4], input_vector))
    p_black = exp(dot(data.iloc[:, 5], input_vector))
    p_dark = exp(dot(data.iloc[:, 6], input_vector))

    total = 1 + p_brown + p_red + p_black
    brown = float(p_brown) / total
    red = float(p_red) / total
    black = float(p_black) / total
    blonde = 1 - brown - red - black

    dark = p_dark / (1 + p_dark)
    light = 1 - dark

    if black == max(brown, red, black, blonde):
        if black > 0.7:
            result = "{0:.2f}".format(
                black * 100) + '% probability of black hair'
        else:
            if dark > 0.5:
                result = "{0:.2f}".format(
                    black * 100) + '% probability of black hair'
                ...
                result = "{0:.2f}".format(
                    black * 100) + '% probability of dark brown/black hair'
    elif blonde == max(brown, red, black, blonde):
        if blonde > 0.7:
            if light > 0.95:
                result = "{0:.2f}".format(
                    blonde * 100) + '% probability of dark blond hair'
            elif light < 0.95:
                result = "{0:.2f}".format(
                    blonde * 100) + '% probability of blond/dark blond hair'
        else:
            if light > 0.9:
                result = "{0:.2f}".format(
                    blonde * 100) + '% probability of blond/dark blond hair'
                ...
                result = "{0:.2f}".format(
                    blonde * 100) + '% probability of dark blond/brown hair'
    elif brown == max(brown, red, black, blonde):
        if brown > 0.7:
            if light > 0.8:
                result = "{0:.2f}".format(
                    brown * 100) + '% probability of brown hair'
            elif light < 0.8:
                result = "{0:.2f}".format(
                    brown * 100) + '% probability of brown/dark brown hair'
        else:
            if light > 0.8:
                result = "{0:.2f}".format(
                    brown * 100) + '% probability of brown/dark brown hair'
                ...
                result = "{0:.2f}".format(
                    brown * 100) + '% probability of dark brown/black hair'
    elif red == max(brown, red, black, blonde) and red > 0.7:
        result = "{0:.2f}".format(red * 100) + '% probability of red hair'
    else:
        result = 'Inconclusive'
